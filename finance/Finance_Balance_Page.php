<?php
/**
 * CloudCup — Admin Finance: Balance Sheet
 * -------------------------------------------------------------
 * Same data layer as the standalone Finance module's Balance Sheet
 * tab (../finance/finance_balance.php), rendered inside the admin
 * panel's own sidebar/topbar/layout instead of the Finance portal's.
 * Snapshot as of today — not date-range filtered like the other
 * finance tabs.
 * -------------------------------------------------------------
 */
require __DIR__ . '/../finance/includes/finance_data.php'; // session/role check + $pdo + all KPI vars + money()

$active_page = 'finance-balance';
$full_name   = $_SESSION['full_name'] ?? 'Admin';

/* -----------------------------------------------------------
   Extra access gate — the Balance Sheet exposes owner capital,
   draws, and retained earnings, so it stays restricted to
   Admin/Manager (owner-level) accounts only, same as the
   standalone Finance module's version of this page.
----------------------------------------------------------- */
if (!in_array(strtolower($_SESSION['role'] ?? ''), ['admin', 'manager'], true)) {
    ...
    <title>Restricted — CloudCup Admin</title>
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Balance Sheet — CloudCup Admin</title>
<link rel="stylesheet" href="../css/admin_page.css">
<link rel="stylesheet" href="../finance/css/finance.css">
</head>
<body>
...
if (file_exists(__DIR__ . '/Sidebar_Admin.php')) {
  ...
  echo '<div style="background:#FEF3C7;border-bottom:1px solid #F59E0B;padding:10px 32px;font-size:13px;color:#92400E">'
...
<div class="main">
  ...
  <div class="content">
    ...
    <div class="section-header" style="margin-top:0;"><h2>Balance Sheet</h2><a class="btn-export" href="../finance/finance_export.php?report=balance&<?= htmlspecialchars($rangeQuery) ?>">⭳ Export to Excel</a><div class="line"></div></div>
