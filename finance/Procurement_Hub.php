...
<head>
  ...
  <style>
    .req-card{border:1px solid var(--pr-rule,#e5dacc);border-radius:12px;padding:18px;margin-bottom:14px;background:var(--pr-card,#fdfbf7);}
    .req-card-head{display:flex;justify-content:space-between;align-items:baseline;margin-bottom:8px;gap:12px;}
    .req-items{margin:10px 0;font-size:13px;color:var(--pr-mute,#93806f);}
    ...
    .po-form-row{display:flex;gap:10px;flex-wrap:wrap;margin:12px 0 2px;}
    .po-form-row select{flex:1;min-width:220px;}

    /* ---- unified field styling: selects + text inputs inside request cards ---- */
    .req-card select,
    .req-actions input[type=text]{
      appearance:none;-webkit-appearance:none;-moz-appearance:none;
      border:1px solid var(--pr-rule,#e5dacc);
      background:var(--pr-card,#fdfbf7) url("data:image/svg+xml;charset=UTF-8,%3Csvg xmlns='http://www.w3.org/2000/svg' width='10' height='6'%3E%3Cpath d='M0 0l5 6 5-6z' fill='%2393806F'/%3E%3C/svg%3E") no-repeat right 13px center;
      background-size:10px 6px;
      border-radius:8px;
      padding:10px 34px 10px 12px;
      font-family:inherit;
      font-size:13.5px;
      color:var(--pr-ink,#3c2317);
      min-height:40px;
      line-height:1.3;
      cursor:pointer;
    ...
    .req-card select:hover,
    .req-actions input[type=text]:hover{border-color:#D8CBB9;}
    .req-card select:focus,
    .req-actions input[type=text]:focus{
      outline:none;border-color:var(--pr-teal,#628e90);
      box-shadow:0 0 0 3px rgba(98,142,144,.16);
    }
    .req-actions input[type=text]::placeholder{color:var(--pr-faint,#b9a895);}

    /* ---- buttons ----
       Scoped to their containers (.po-form-row / .req-actions) so these rules
       out-specificity the plain .btn-save defined in inventory_management.css,
       regardless of <link>/<style> load order. */
    .po-form-row .btn-save,.req-actions .btn-save,
    .po-form-row .btn-return,.req-actions .btn-return,
    .po-form-row .btn-reject,.req-actions .btn-reject{
      border-radius:8px !important;padding:10px 18px !important;font-size:13px;font-weight:600;font-family:inherit;
      cursor:pointer;white-space:nowrap;background-image:none !important;
      transition:background .15s ease,box-shadow .15s ease,border-color .15s ease,
                 transform .05s ease,opacity .15s ease;
    }
    .po-form-row .btn-save,.req-actions .btn-save{
      background:var(--pr-cocoa,#3c2317) !important;color:var(--pr-cream,#f5efe6);border:1px solid var(--pr-cocoa,#3c2317);
      box-shadow:0 1px 0 rgba(60,35,23,.18);
    }
    .po-form-row .btn-save:hover,.req-actions .btn-save:hover{background:#4E3122 !important;box-shadow:0 4px 12px -4px rgba(60,35,23,.45);transform:none;}
    .po-form-row .btn-save:active,.req-actions .btn-save:active{transform:translateY(1px);box-shadow:0 1px 0 rgba(60,35,23,.18);}
    .po-form-row .btn-save[disabled],.req-actions .btn-save[disabled]{opacity:.45;cursor:not-allowed;box-shadow:none;transform:none;}
    .po-form-row .btn-return,.req-actions .btn-return{background:#fff4e5 !important;color:#b45300;border:1px solid #f0d6a8;}
    .po-form-row .btn-return:hover,.req-actions .btn-return:hover{background:#ffe9c9 !important;border-color:#e8c48a;}
    .po-form-row .btn-return:active,.req-actions .btn-return:active{transform:translateY(1px);}
    .po-form-row .btn-reject,.req-actions .btn-reject{background:#fdeaea !important;color:#c0392b;border:1px solid #f3c6c6;}
    .po-form-row .btn-reject:hover,.req-actions .btn-reject:hover{background:#fcdcdc !important;border-color:#eab3b3;}
    .po-form-row .btn-reject:active,.req-actions .btn-reject:active{transform:translateY(1px);}
    ...
    .pr-panel .pr-field{display:flex;align-items:center;justify-content:space-between;gap:8px;width:100%;
    ...
    .pr-panel button.pr-field:hover{border-color:var(--pr-teal);}
            ...
            <div class="req-card">
              ...
              <form method="POST" action="?tab=assign_supplier" class="swal-confirm-form" data-swal-text="Send this request to the selected supplier to check stock and quote a price?" data-swal-confirm="Yes, send it">
                ...
                <div class="po-form-row">
                  ...
                  <button type="submit" class="btn-save">Assign Supplier</button>
                </div>
              </form>
            </div>
          <?php endforeach; endif; ?>
