...
$is_admin_session   = isset($_SESSION['user_id'], $_SESSION['role']) && $_role_lc === 'admin';

if (!$is_staff_session && !$is_manager_session && !$is_admin_session) {
...
$role      = $_SESSION['role'];
$is_admin  = $is_manager_session || $is_admin_session; // kept as $is_admin: drives the full-CRUD vs read-only view below
...
// ── Staff: Adjust Quantity (AJAX, no full page reload) ─────────────────────
...
// decisions, not stock counts. Every adjustment requires a reason and is
// written to inventory_log under the staff member's own employee_id, so it
// shows up in the Manager's activity feed automatically — no separate
// approval step needed for routine counts.
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['act'] ?? '') === 'staff_adjust_qty') {
  header('Content-Type: application/json');
  ...
  $inv_id      = (int)($_POST['inventory_id'] ?? 0);
  $new_qty     = isset($_POST['new_quantity']) && $_POST['new_quantity'] !== '' ? (float)$_POST['new_quantity'] : null;
  $reason_key  = trim($_POST['reason'] ?? '');
  $reason_note = trim($_POST['note'] ?? '');
  ...
  if ($new_qty === null || $new_qty < 0 || $new_qty > 9999) {
    echo json_encode(['success' => false, 'message' => 'Enter a valid quantity (0–9999).']);
    exit;
  }
  if (!isset($reason_labels[$reason_key])) {
    echo json_encode(['success' => false, 'message' => 'Please select a reason for this adjustment.']);
    exit;
  }

  $item = mysqli_fetch_assoc(mysqli_query(
    $conn,
    "SELECT item_name, quantity FROM inventory WHERE inventory_id = $inv_id"
  ));
  if (!$item) {
    echo json_encode(['success' => false, 'message' => 'Item not found.']);
    exit;
  ...
  if ($diff == 0) {
    echo json_encode(['success' => false, 'message' => 'New quantity is the same as the current quantity — nothing to update.']);
    exit;
  }

  $stmt = mysqli_prepare($conn, 'UPDATE inventory SET quantity = ? WHERE inventory_id = ?');
  mysqli_stmt_bind_param($stmt, 'di', $new_qty, $inv_id);
  mysqli_stmt_execute($stmt);

  $ct   = $diff > 0 ? 'restock' : 'adjustment';
  $note = $reason_labels[$reason_key] . ($reason_note !== '' ? ' — ' . $reason_note : '') . ' (reported by ' . $full_name . ')';
  $log  = mysqli_prepare(
    $conn,
    'INSERT INTO inventory_log (inventory_id, employee_id, change_type, qty_change, notes) VALUES (?,?,?,?,?)'
  );
  mysqli_stmt_bind_param($log, 'iisds', $inv_id, $user_id, $ct, $diff, $note);
  mysqli_stmt_execute($log);
  log_activity($conn, 'inventory', 'updated', 'inventory_item', $inv_id, 'Quantity adjusted for ' . $item['item_name'] . ' from ' . $old_qty . ' to ' . $new_qty . '.');

  echo json_encode([
    'success' => true,
    'message' => 'Stock updated for "' . $item['item_name'] . '" (' . ($diff > 0 ? '+' : '') . $diff . ').'
  ]);
  exit;
}
...
if ($is_admin && $_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($act === 'add') {
    ...
    if ($item_name && $unit) {
      ...
      if ($dup_exists) {
        ...
        mysqli_stmt_execute($log);
        log_activity($conn, 'inventory', 'added', 'inventory_item', $new_id, 'Added ' . $item_name . ' with opening quantity ' . $quantity . '.');
  ...
  if ($act === 'edit') {
    ...
    if ($inv_id && $item_name && $unit) {
      ...
      log_activity($conn, 'inventory', 'updated', 'inventory_item', $inv_id, 'Updated item details for ' . $item_name . '.');
...
<body>
  ...
  <?php
  // ── SIDEBAR ──────────────────────────────────────────────────────────────────
  // Render navigation for the authenticated management role.
  if ($is_admin_session && file_exists(__DIR__ . '/Sidebar_Admin.php')):
    require_once __DIR__ . '/Sidebar_Admin.php';
  elseif ($is_manager_session && file_exists(__DIR__ . '/../manager/Sidebar_Manager.php')):
    require_once __DIR__ . '/../manager/Sidebar_Manager.php';
  else: ?>
    <?php
  ...
  <div class="main">
    ...
    <div class="content">
                    ...
                    <td>
                      <div class="action-group">
                        <?php if ($is_admin): ?>
                          ...
                          <div class="action-btn danger" title="Archive"
                            onclick="confirmRemoveItem(<?= $item['inventory_id'] ?>, '<?= htmlspecialchars(addslashes($item['item_name'])) ?>')"><i data-lucide="archive"></i></div>
                        <?php else: ?>
      ...
      <div class="panel" style="margin-top:20px">
        <div class="panel-toolbar" style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px">
          <div style="display:flex;align-items:center;gap:8px;font-weight:600;font-size:15px;color:var(--text-dark,#241f19)">
            <i data-lucide="archive" style="width:16px;height:16px;color:var(--text-light)"></i>
            Removed Items
      ...
      <div class="modal">
        ...
        <form id="form-staff-adjust">
          ...
          <div class="modal-footer">
            ...
            <button type="submit" class="btn-save">Save Adjustment</button>
          </div>
        </form>
      </div>
    </div>
